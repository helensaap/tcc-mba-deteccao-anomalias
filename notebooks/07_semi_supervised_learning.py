"""
FASE 7: SEMI-SUPERVISED LEARNING COM 15K IMAGENS NÃO-ROTULADAS
================================================================

Objetivo: Explorar as 15.336 imagens diárias não-rotuladas para melhorar acurácia

Estratégia:
1. Carregar modelo treinado (Fase 6: 52.94% test accuracy)
2. Fazer pseudo-labeling nas 15k imagens não-rotuladas
3. Implementar MixMatch para consistency regularization
4. Retreinar com dados aumentados (107 labeled + 15k pseudo-labeled)
5. Validar melhoria
...
print("\n" + "="*80)
print("FASE 7: SEMI-SUPERVISED LEARNING COM 15K IMAGENS")
print("="*80)
...
CONFIG = {
    ...
    'data_dir': 'data',
    'pseudo_label_threshold': 0.85,  # Confidence mínima para pseudo-label
    'lambda_u': 1.0,  # Peso do unlabeled loss
...
print(f"   Lambda (unlabeled weight): {CONFIG['lambda_u']}")
...
# ============================================================================
# DAILY IMAGE LOADER - CARREGAR 15K IMAGENS NÃO-ROTULADAS
# ============================================================================

class DailyImageLoader:
    """Carrega imagens diárias não-rotuladas (15.3k imagens)"""
...
# ============================================================================
# PSEUDO-LABELING - ATRIBUIR LABELS AUTOMÁTICOS
# ============================================================================
...
# Preparar transforms para consistência
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])

# Pseudo-label as imagens usando modelo treinado
pseudo_labeled_data = []
pseudo_confidences = []

print("\n🔮 Gerando pseudo-labels (isso pode levar alguns minutos)...")
...
with torch.no_grad():
    for i, img_data in enumerate(tqdm(unlabeled_images, desc="Pseudo-labeling", total=len(unlabeled_images))):
        ...
        # Usar modelo para prever label
        visual_feat = visual_model(img_tensor)
        # Para imagens sem sensor, usar zero tensor
        ...
        # Guardar apenas se confidence > threshold
...
# ============================================================================
# DATASET COM LABELED + PSEUDO-LABELED DATA
# ============================================================================

class MixMatchDataset(Dataset):
    """Dataset que combina labeled + pseudo-labeled data com MixMatch"""
    ...
    def __getitem__(self, idx):
        if idx < len(self.labeled_df):
            ...
            is_labeled = torch.tensor(0.0)  # Marca como não-labeled

            # Sempre com augmentação forte para não-labeled
            img_tensor = self.strong_augmentation(img_array)
            # Sensor fictício (zeros) para imagens diárias
...
# Usar todos os 107 labeled para semi-supervised
...
combined_dataset = MixMatchDataset(labeled_df, pseudo_labeled_data, augment=True)
...
# Optimizer com learning rates diferentes
...
# Loss function com class weighting
class_weights = torch.FloatTensor([1.0, 1.5]).to(device)  # Dar mais peso a stress
...
# ============================================================================
# TREINAMENTO COM SEMI-SUPERVISED LEARNING
# ============================================================================

print("\n" + "="*80)
print("🎯 INICIANDO SEMI-SUPERVISED TRAINING")
print("="*80)
...
best_model_path = Path("models") / "best_model_semi_supervised.pt"
...
# Salvar histórico
with open(results_dir / "07_semi_supervised_history.json", 'w') as f:
    json.dump({
...
plt.subplot(1, 2, 2)
plt.text(0.1, 0.9, 'RESUMO SEMI-SUPERVISED LEARNING', fontsize=14, fontweight='bold', transform=plt.gca().transAxes)
plt.text(0.1, 0.8, f'Labeled samples: {len(labeled_df)}', fontsize=11, transform=plt.gca().transAxes)
plt.text(0.1, 0.7, f'Pseudo-labeled: {len(pseudo_labeled_data)}', fontsize=11, transform=plt.gca().transAxes)
...
plt.text(0.1, 0.3, f'Pseudo-label threshold: {CONFIG["pseudo_label_threshold"]}', fontsize=11, transform=plt.gca().transAxes)
plt.text(0.1, 0.2, f'Lambda (SSL weight): {CONFIG["lambda_u"]}', fontsize=11, transform=plt.gca().transAxes)
plt.axis('off')

plt.tight_layout()
plt.savefig(results_dir / "07_semi_supervised_training.png", dpi=100)
...
print("\n" + "="*80)
print("📋 RESUMO: SEMI-SUPERVISED LEARNING")
print("="*80)
print(f"""
...
├─ Pseudo-labeled: {len(pseudo_labeled_data)} (threshold: {CONFIG['pseudo_label_threshold']})
└─ Total training: {len(combined_dataset)}

TREINAMENTO:
├─ Epochs: {epoch+1}/{CONFIG['num_epochs']} (early stopping)
├─ Learning rate: {CONFIG['learning_rate']}
├─ Lambda (unlabeled weight): {CONFIG['lambda_u']}
...
PRÓXIMOS PASSOS:
→ Avaliar modelo em test set
→ Comparar com Fase 6 (52.94%)
→ Espera-se melhoria significativa com 15k amostras adicionais!
""")
